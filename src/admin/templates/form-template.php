...
function PMP_handle_form_submission()
{
    if (isset($_POST['import_template'])) {
        if (isset($_POST['pmp_nonce']) && wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['pmp_nonce'])), 'pmp_import_action')) {
            import_template();
        }
    } elseif (isset($_POST['create_page'])) {
        if (isset($_POST['pmp_create_page_nonce']) && wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['pmp_create_page_nonce'])), 'pmp_create_page_action')) {
            create_page();
    ...
    if (isset($_POST['create_page']) || isset($_POST['update_page']) || isset($_POST['delete_page'])) {
        // Get the URL from wherever you're setting it now
        $redirect_url = isset($_POST['create_page']) || isset($_POST['update_page'])
            ? get_permalink($page_id) // Use the appropriate page ID variable
            : admin_url('admin.php?page=page-management');

        // Add a success parameter to show a notice after redirect
        $redirect_url = add_query_arg('pmp_status', 'success', $redirect_url);
...
function create_page()
{
    if (isset($_POST['pmp_create_page_nonce']) || wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['pmp_create_page_nonce'])), 'pmp_create_page_action')) {
        ...
        $redirect_url = get_permalink($new_page_id); // or whatever URL
?>
...
function update_page()
{
    if (isset($_POST['pmp_create_page_nonce']) || wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['pmp_create_page_nonce'])), 'pmp_create_page_action')) {
        ...
        $redirect_url = get_permalink($page_id); // or whatever URL
    ?>
        <script type="text/javascript">
            window.location.href = "<?php echo esc_url($redirect_url); ?>";
        </script>
...
function delete_page()
{
    ...
    ?>
        <script type="text/javascript">
            window.location.href = "<?php echo esc_url(admin_url('admin.php?page=page-management')); ?>";
        </script>
    <?php
        exit;
    }
